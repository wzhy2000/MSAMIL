...
def collate_MIL(batch):
	...
	label = torch.LongTensor([int(item[1]) for item in batch])
	return [img, label]
...
def get_split_loader(split_dataset, training = False, testing = False, weighted = False):
	...
	if not testing:
		...
		ids = np.random.choice(np.arange(len(split_dataset), int(len(split_dataset)*1)), replace = False)
...
def generate_split(cls_ids, val_num, samples, n_splits=5, seed=7, label_frac=1.0, method='montecarlo'):
        ...
        for i in range(n_splits):
            ...
            for c in range(len(val_num)):
                ...
                if label_frac == 1:
                    ...
                    sampled_train_ids.extend(np.random.choice(remaining_ids, sample_num, replace=False))

            yield sampled_train_ids, all_val_ids

    elif method == 'balanced':
        idx_label_list = []
        for label, indices in enumerate(cls_ids):
            for idx in indices:
                idx_label_list.append([idx, label])
        idx_label_list.sort(key=lambda x: x[0])
        patient_label_df = pd.DataFrame(idx_label_list, columns=['Patient_ID', 'label'])
				

        skf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=seed)

        for train_idx, val_idx in skf.split(patient_label_df['Patient_ID'].tolist(), patient_label_df['label'].tolist()):
            sampled_train_ids = patient_label_df.iloc[train_idx]['Patient_ID'].tolist()
            all_val_ids = patient_label_df.iloc[val_idx]['Patient_ID'].tolist()

            yield sampled_train_ids, all_val_ids

    else:
        raise ValueError("Invalid method. Choose 'montecarlo' or 'balanced'.")
