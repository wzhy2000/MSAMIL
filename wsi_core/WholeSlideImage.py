...
class WholeSlideImage(object):
    def __init__(self, path, annotation=None):
        ...
        self.patient_id = self.name.split('_')[0]
        ...
        if self.annotation:
            self.annotation_json_path = os.path.join(annotation, f'{self.patient_id}.json')
            self.tumor_polygons = self.get_tumor_area_json(self.annotation_json_path)

        self.max_tumor_patch_num = 1000
        self.max_normal_patch_num = 1000
        self.tumor_patch_num = 0
        self.normal_patch_num = 0
    ...
    @staticmethod
    def get_tumor_area_json(annotation_json_path):
        tumor_polygons = []
        ...
        for shape in annotations['shapes']:
            if shape['label'] == 'Tumor':
                coordinate_list = []
                for coordinate in shape['points']:
                    border_coordinate = (float(coordinate[0]), float(coordinate[1]))
                    coordinate_list.append(border_coordinate)

                poly = Polygon(coordinate_list)
                tumor_polygons.append(poly)
        return tumor_polygons
    

    @staticmethod
    def search_tumor_label(tile_loc, tumor_areas):
        p = Polygon(((tile_loc[0], tile_loc[1]), (tile_loc[2], tile_loc[1]), (tile_loc[2], tile_loc[3]), (tile_loc[0], tile_loc[3])))
        tumor_label = 0
        for tumor_area in tumor_areas:
            if p.intersects(tumor_area):
                tumor_label = 1
                break
        return tumor_label
    
    @staticmethod
    def get_loc(x, y, patch_size):
        return [x, y, x + patch_size, y + patch_size]
    

    def isLightPatch(self, patch):
        gray_tile = cv2.cvtColor(patch, cv2.COLOR_RGB2GRAY)
        adaptive_thresh = cv2.adaptiveThreshold(gray_tile, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                                        cv2.THRESH_BINARY, 11, 2)
        light_background_ratio = np.sum(adaptive_thresh == 255) / (gray_tile.shape[0] * gray_tile.shape[1])
        return light_background_ratio > 0.8
    ...
    def process_coord_candidate(self, coord, contour_holes, ref_patch_size, cont_check_fn):
        x, y = coord
        patch_PIL = self.wsi.read_region((x, y), 0, (ref_patch_size, ref_patch_size))
        patch = np.array(patch_PIL)
        if self.isLightPatch(patch):
            return None

        if self.annotation:
            tile_loc = self.get_loc(x, y, ref_patch_size)
            in_tumor_region = self.search_tumor_label(tile_loc, self.tumor_polygons)
            if in_tumor_region:
                if self.tumor_patch_num<self.max_tumor_patch_num:
                    self.tumor_patch_num += 1
                    # print("Tumor Patch:", self.tumor_patch_num)
                else:
                    return None
            else:
                if self.normal_patch_num<self.max_normal_patch_num:
                    self.normal_patch_num += 1
                    # print("Normal Patch:", self.normal_patch_num)
                else:
                    return None
